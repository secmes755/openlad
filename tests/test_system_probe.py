...
def test_cpu_mode_by_memory():
    ...
    rec2 = recommend_context(None, {"total_mb": 8192, "available_mb": 4000})
    assert rec2["llm_ctx"] == 8192


def test_minimum_context_floor():
    gpu = {"name": "min", "vram_total_mb": 1024, "vram_free_mb": 512}
    rec = recommend_context(gpu, {"total_mb": 4096, "available_mb": 2000})
    assert rec["llm_ctx"] == MIN_LLM_CONTEXT
