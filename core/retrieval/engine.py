...
Output ONLY a JSON object: {"type": "deep_research"} or {"type": "traditional"}"""
    ...
    def query(self, query_text: str, tenant_id: str,
        ...
        if query_type == "deep_research":
            ...
            # DEGRADED-DOC PREMISE: this path bypasses SegmentMerger, so it does
            # not render ingest warnings. `degraded` now means *content* may be
            # incomplete (page loss, OCR/transcription failure, parse loss,
            # embedding loss); declaration/config mismatches are stored separately
            # and do not flip status. Spec facts are only injected when present in
            # the index and self-verified against page text, so this path never
            # claims completeness by itself — but any future text-damaging degraded
            # cause should be reflected here too, not only in SegmentMerger.
